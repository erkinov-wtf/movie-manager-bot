...
import (
	...
	keyboards2 "github.com/erkinov-wtf/movie-manager-bot/pkg/keyboards"
...
func (*DefaultHandler) Start(context telebot.Context) error {
	...
	if err := database.DB.Where("id = ?", context.Sender().ID).First(&existingUser).Error; err != nil {
		...
		menu := keyboards2.LoadMenuKeyboards(context.Bot())
...
func (h *DefaultHandler) handleStartCallback(context telebot.Context) error {
	...
	keyboard := keyboards2.LoadTokenRegistrationKeyboard(context.Bot(), h)
